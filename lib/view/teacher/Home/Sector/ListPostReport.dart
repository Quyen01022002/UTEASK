...
class ListPostReportScreen extends StatefulWidget {
  const ListPostReportScreen({super.key});

  @override
  State<ListPostReportScreen> createState() => _ListPostReportScreenState();
}

class _ListPostReportScreenState extends State<ListPostReportScreen> {

  final HomeGroupController homeGroupController =
  Get.put(HomeGroupController());
  final ScrollController _scrollController = ScrollController();
  List<ReportPostResponse> _posts = [];
  ...
  Future<void> _fetchReport() async {
    ...
    List<ReportPostResponse>? response;
    //repose = load giá trị listpost dưới controller
    response = await homeGroupController.loadListReportPost(context);
    if (response!= null && response.length!=0) {
      setState(() {
        //set pagenumber ++
        homeGroupController.pageReport.value++;
